...
| UnseenAdjustmentsModal | Complete (post-login modal listing unseen point adjustments; positive triggers canvas-confetti emoji burst, negative-only triggers full-screen red hue + horizontal shake) |
| UserForm | Complete |
...
- `SlimChoreCard` for Today — single contextual button on the right (Start / Submit / Resume; "Waiting…" for submitted; "Paused — see My Chores" for parent_paused). Pause is intentionally absent — child must visit My Chores tab to pause. `in_progress` cards sort to the top of the list and use a cyan tint (`bg-cyan-500/15`) to make active work visually distinct from not-yet-started chores.
...
**Post-login overlay — `UnseenAdjustmentsModal`:**

Mounted in `ChildView` over the dashboard whenever `GET /adjustments/unseen` returns one or more rows. Lists each adjustment as a green (positive) or red (penalty) card with reason and parent name. Dismissed via "Got it" → `PATCH /adjustments/seen` marks all as seen.

Animations (defined in `index.css`):

- Modal scale-fades in (200ms). Cards inside stagger in 50ms apart; the points value pops (`scale 0.8 → 1.1 → 1`) 150ms after each card lands.
- **Negative-only adjustments:** backdrop is `bg-red-500/25` (full-screen red hue) instead of the default `bg-black/70`. Modal shakes horizontally on entry (~6px, 500ms, fires once after the scale-fade lands). Red hue persists for the full duration the modal is open, then fades out over 400ms on dismiss.
- **Positive-only and mixed (positive + negative):** default backdrop, no shake, no hue. `canvas-confetti` emoji-particle burst (🔥 + 💥) fires from `origin: { y: 0.6 }` ~600ms after mount — `particleCount: 60`, `spread: 90`, `scalar: 2`. Mixed adjustments fire the same positive burst; the per-card red hue on penalty cards is sufficient for negative context.

On dismiss: modal opacity fades 200ms, backdrop opacity fades 400ms, then unmounts. The `markAllSeen` mutation invalidates only `['dashboard', 'child']` (not `['adjustments', 'unseen']`) so the parent doesn't yank the modal mid-fade via query refetch.
...
- **Pool chore overhaul — inline status + recurrence + team flag** — remove standalone pool panel from Chores tab. Pool button on each chore card uses the same recurrence expansion as child buttons (daily/weekly/monthly + one-time). One-time button shows count of current pool instances with color change indicating active pool presence. Pool button color changes when recurring schedule is active. Removal via same modal pattern as child schedules. Chore info modal updated: pool recurrence shown in schedule section alongside child schedules, plus "Currently in Pool: [count]" in top-right header area. Pool + child recurrence can coexist on the same chore — parent's call. **Team chore flag** (boolean on `chores`) — when set, claiming the pool chore creates a new assignment for the claimant but leaves the pool entry intact, so siblings can also claim and earn. Each claim is an independent assignment with its own submit/approve lifecycle (parent approves each separately). Points are per-claimant — chore form displays "X pts each" when team flag is on, but the stored `points` value is unchanged. Pool entry expires/regenerates with the recurrence period (e.g., daily team chore replaces every morning). Backend changes: `team_chore` column on `chores`, `chore_schedules.child_id` must be nullable to support pool-targeted schedules, `claimAssignment` branches on team flag (insert new row vs. update pool row).
...
- **One-tap approve on parent dashboard** — add a full-width Approve button at the bottom of `ApprovalCard` with a 2-second Undo countdown (mirror the pattern in `ChoreCard.jsx` — `useRef` for the interval, fire on unmount if still pending). Approve button needs `e.stopPropagation()` so taps don't bubble to the card's `onClick`. Tapping anywhere else on the card still opens the existing approval modal; the countdown continues regardless of modal state. If the countdown completes while the modal is open, the modal stays open until the parent closes it, but its Approve/Reject/Dismiss buttons disable once the auto-approve fires (otherwise a tap on Reject/Dismiss would 4xx against an already-accepted assignment). Reject and Dismiss flows unchanged — still in the modal because Reject requires a comment and Dismiss is rare. Reduces the dominant approve path from 2 taps to 1.
- **Lottie fire animation for positive burst (alternative to confetti)** — current shipped positive burst uses `canvas-confetti` with 🔥 + 💥 emoji particles. Worth exploring a Lottie-based fire animation for higher visual fidelity — designer-quality flame effect rendered from a JSON file rather than emoji particles. Adds `@lottiefiles/react-lottie-player` (~50KB) plus a fire JSON (need to source/license). Decision deferred until canvas-confetti version has been used long enough to evaluate whether the upgrade is worth the bundle weight and sourcing effort.
- **Funded rewards on parent dashboard** — surface `funded` rewards in the parent dashboard Requests panel as a new card type (`DashFundedCard`) alongside chore submissions, pending reward requests, and refund requests. Indigo border to distinguish from existing green/amber/rose. Position: top of the Requests panel (highest urgency — kid earned it, parent owes the thing). Card content: reward name + stacked contributor avatars (max 3, then "+N"); points_required hidden on the dashboard (lives in Rewards tab if needed). Badge text: "Ready to Redeem". Tap → reuses the existing reward modal from `ParentRewardsTab` (which already has the redeem flow). Backend: needs `fundedRewards` added to `/dashboard/parent` response (see odin-reference.md).
